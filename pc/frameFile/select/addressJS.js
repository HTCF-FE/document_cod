...
$.fn.sjld = function(shenfen,chengshi,quyu){
	...
	//包含下拉数据的ul标签
			sfs = shenfen+' .optionCopy',
			css = chengshi+' .optionCopy',
			qys = quyu+' .optionCopy',
	...
	//大数组中的第一个城市的区域的数据  （市级）
	var csgsmr = sfgsmr[0].cityList;

	//大数组中的第一个城市的第一个区域的详细地区数据  （区级）
	var qygsmr = csgsmr[0].areaList,
	...
	//设置初始化时默认显示的数据
	$(sfp).text(sfgsmr[0].name).addClass('selectResult');
	$(csp).text(csgsmr[0].name).addClass('selectResult');
	$(qyp).text(qygsmr[0]).addClass('selectResult');
	

	//调用设置初始化时默认的下拉列表并插入到页面中的方法
	sfgsmrAppend(sfgsmr);  //省
	csgsmrAppend(csgsmr);  //市
	qygsmrAppend(qygsmr);  //区


	//设置初始化时默认的下拉列表并插入到页面中的方法
	function sfgsmrAppend(sfgsmr){  //省
	...
	function csgsmrAppend(csgsmr){ //市
	...
	function qygsmrAppend(qygsmr){  //区
	...
	/*
			省级的点击事件
	 */
	$(shenfen).on('click','.optionCopy li',function(e){

		var dqsf = $(this).text(); //获得文案
		$(shenfen).find('p').text(dqsf).addClass('selectResult');

		$(this).parent().slideUp(200);

		$(shenfen).find('p').attr('title',dqsf); //设置title属性为选中的文案

		sfgSave = $(this).index(); //保存该条li的索引
		
		var csgs = provinceList[sfgSave].cityList; //获取对应的市级数据
		var csgs2 = provinceList[sfgSave].cityList[0].areaList; //获取对应的第一条市级数据的区级数据
		$(chengshi).find('ul').text(''); //将市级数据的下拉列表清空
		
		//重新设置市级数据的下拉列表并插入
		csgsmrAppend(csgs);
	
		$(quyu).find('ul').text('');  //清空区级的下拉列表
	...
	市级点击事件  采用动态绑定事件的方法
	*/
	$(chengshi).on('click','.optionCopy li',function(e){

		var dqcs = $(this).text(); //获得文案

		$(chengshi).find('p').text(dqcs).addClass('selectResult');

		$(this).parent().slideUp(200);

		$(chengshi).find('p').attr('title',dqcs);  //设置title属性值选中的文案
		
		csgSave = $(this).index(); //保存该条li的索引

		var qygs = provinceList[sfgSave].cityList[csgSave].areaList;  //获取对应的第一条市级数据的区级数据
		
		$(quyu).find('ul').text(''); //将区级数据的下拉列表清空
	...
	/*
		区级的点击事件  采用动态绑定事件的方法
		*/
	$(quyu).on('click','.optionCopy li',function(e){

		var dqqy = $(this).text(); //获得文案
		$(quyu).find('p').text(dqqy).addClass('selectResult');

		$(this).parent().slideUp(200);

		$(quyu).find('p').attr('title',dqqy); //设置title属性值选中的文案
...
var provinceList = [
	...
	{name:'香港', cityList:[{name:'香港', areaList:['香港']}]},
